...
enum TransactionType: String, CaseIterable, Codable {
    ...
    static func detect(from lowercasedDetails: String) -> TransactionType {
        ...
        if d.contains("od loan") || d.contains("overdraft") || d.contains("m-pesa overdraw") { return .overdraft }
        ...
        if d.contains("globalpay") || d.contains("card pay bill") { return .cardPayment }
        if d.contains("transfer of funds charge") { return .transferFee }
        if d.contains("withdrawal charge") { return .withdrawalCharge }
        if d.contains("pay bill charge") || d.contains("pay merchant charge") { return .paybillCharge }
        ...
        if d.contains("recharge for customer to 150501") || d.contains("safaricomhome") { return .payBill }
        if d.contains("bundle purchase") || d.contains("recharge for customer") { return .bundlePurchase }
        if d.contains("airtime") { return .airtime }
        if d.contains("payment to small business") { return .smallBusinessPayment }
        if d.contains("customer transfer to") || d.contains("customer send money") { return .sendMoney }
    ...
    var isExpense: Bool {
        switch self {
        case .receiveMoney, .businessPayment, .internationalReceive,
             .agentDeposit, .unitTrustWithdraw, .mShwariWithdraw,
             .ziidiSell, .ziidiDividend:
            return false
        default:
            return true
        }
